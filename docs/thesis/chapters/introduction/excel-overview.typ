...
The _Excel PivotTable_ is a powerful tool for summarising and analysing data from external sources @microsoft_overview_2025-2. Unlike normal tables, _PivotTables_ are dynamic in layout, and allow for reorganisation of the data. It provides a dynamic view of the underlying data source and allows the user to perform operations on the data @microsoft_overview_2025-2. The core feature of _PivotTables_ lies in their ability to perform aggregations as a low-code solution. Without writing a single formula, it is possible to construct comprehensive views of the data. This is made even easier with automatic refreshing, which updates the PivotTable when there is a change in the underlying data model @microsoft_overview_2025-2.

_PivotTables_ support grouping. Dates, for example, can be grouped into months or years, and numeric data can be binned into intervals. These groupings are handled internally and allow for a more compact and meaningful summary @microsoft_overview_2025-2.

It is important to note that PivotTables do not compute data themselves @microsoft_overview_2025-2. Instead, they act as a kind of presentation layer of other datasources. These datasources can be internal or external. Hence, while they could depend on the data and formulas defined elsewhere, they do not themselves perform cell-level computations in the same way as formulas in worksheets or structured references in _Excel Tables_ @microsoft_overview_2025-2.

== Formulae

In Excel, a formula is an expression that calculates the value of a cell @microsoft_overview_2025-1. Every formula begins with an equal sign `=`, which tells Excel that the contents of the cell constitute a formula @microsoft_overview_2025-1. These formulas can contain functions, references, operators, and constants to perform calculations on data within the Excel sheet. These distinct elements all contribute different functionality to the end results:

- Constants are fixed values entered directly into the formula.
- Functions are predefined operations that perform specific calculations on their arguments. For example, `SUM(1;2;3;4;5)` will sum up the arguments. An interesting thing with Excel is that the names and notation of the functions are language dependent @microsoft_overview_2025-1. For instance, in Dutch, the `SUM` function is translated to `SOM`. As well as the notation for the arguments list, where in one language the arguments are split with a semi-colon `;` , another language splits them with a comma `,`.
- Operators are special functions that can be inlined. For example, the plus `+` or minus `-` operators. For the order of operations, Excel follows the PEMDAS rule @microsoft_overview_2025-1.
- References provide pointers to other cells or ranges within the worksheet in the form of the _A1-notation_. This notation targets the rows and columns, where the columns are represented by the alphabet, and the rows by a number. For instance, the cell in the third column, and fourth row, would be `C4`. References can be _relative_ to the cell or _absolute_. _absolute references_ are started with a dollar sign `$`. It is possible to only make the row or column absolute, to create a _hybrid reference_.
