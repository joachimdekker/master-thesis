...
#citeauthor(<sestoft_spreadsheet_2006>) describes a full alternative implementation of spreadsheets, with subsequent master theses expanding upon the work @iversen_runtime_2006 @poulsen_optimized_2007 describe how to efficiently calculate the values in a spreadsheet using the support graph: a more organized version of the functional dependencies used in @cunha_automatically_2010. In comparison with #citeauthor(<cunha_automatically_2010>), these works also consider transitive dependencies in their model. Furthermore, they only represent the calculations, but do not generate code.
...
// Advanced functional programming techniques offer several strategies for handling mutual recursion when transforming high-level definitions into efficient code. Notably, transformation systems (as in Burstall & Darlington) show how multiple mutually recursive functions can be merged into a single recursive structure by introducing an indexing mechanism, thereby streamlining both execution and verification. Complementing this, approaches like the universal recursive function (URF) method simplify mutual recursion analysis by converting separate recursive definitions into a unified recursive function, which aids in formal verification. Furthermore, staged computation techniques—as detailed by Yallop and Kiselyov—employ let-insertion and memoization primitives (e.g., genlet, genletrec, and specialized fixpoint combinators) to generate recursive code that avoids scope extrusion and redundant computation. These methods collectively provide a robust framework that can be adapted to translate complex recursive constructs, such as those found in Excel functions, into efficient and verifiable .NET code.


// Several methodologies have been proposed for the systematic conversion and analysis of mutual recursive functions. For instance, Burstall and Darlington propose a transformation system that consolidates mutually recursive definitions into a single recursive function through an indexing mechanism, thereby facilitating both optimization and formal reasoning (A Transformation System for Developing Recursive Programs, 1977). In parallel, Ushakova and Legalov introduce a verification framework that employs a universal recursive function (URF) to merge multiple recursive entities, enabling robust data-flow analysis and formal correctness proofs (Verification of Programs with Mutual Recursion in Pifagor Language, 2018). Additionally, Yallop and Kiselyov explore staged computation techniques that leverage advanced let-insertion and memoization primitives—such as genlet and genletrec—to generate mutually recursive code while effectively mitigating issues like scope extrusion (Generating mutually recursive definitions, 2019). These approaches collectively offer valuable insights for translating high-level recursive constructs, such as Excel functions, into efficient and verifiable .NET implementations.
